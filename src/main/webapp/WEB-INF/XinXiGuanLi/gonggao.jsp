...
<div>
	<ul class="breadcrumb">
		...
		<li><a href="/gonggao/loginGongGaoPage">公告管理</a></li>
	</ul>
</div>
					...
					<div class="pagination pagination-centered">
					 <div id="tianye-zuo" style="float: left; padding-top: 8px; padding-left: 130px; font-size: 15px; color: blue;">共${page.zongShu }条公告  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  ${page.page }/${page.zongYeShu }页</div>
					  ...
					  <c:choose>
					  	...
					  	<c:otherwise>
					  	<li><a href="/gonggao/loginGongGaoPage?currentpage=1" title="首页">首页</a></li>
					  	</c:otherwise>
					  </c:choose>
						
						<c:if test="${page.prevPages!=null}">
							<c:forEach items="${page.prevPages}" var="num">
								<li><a href="/gonggao/loginGongGaoPage?currentpage=${num}"
						...
						<c:if test="${page.nextPages!=null}">
							<c:forEach items="${page.nextPages}" var="num">
								<li><a href="/gonggao/loginGongGaoPage?currentpage=${num}" title="${num}">
								${num} </a></li>
							</c:forEach>
						</c:if>
						<c:if test="${page.zongYeShu !=null}">
							<c:choose>
						  	...
						  	<c:otherwise>
						  	<li><a href="/gonggao/loginGongGaoPage?currentpage=${page.zongYeShu}" title="尾页">尾页</a></li>
						  	</c:otherwise>
						    </c:choose>
					    </c:if>
						<c:if test="${page.zongYeShu == null}">
					  ...
					  <div id="tianye-you" style="float: right; padding-top: 3px; margin-right: 250px; font-size: 15px; color: blue;">
					  	<form action="/gonggao/loginGongGaoPage" method="post">
	...
	<div class="modal hide fade" id="addAfficheDiv">
		<form action="/gonggao/addGongGao" enctype="multipart/form-data" method="post" onsubmit="return addAfficheFunction();">
	 ...
	 <div class="modal hide fade" id="modifyAfficheDiv">
		<form action="/gonggao/updateGongGao" enctype="multipart/form-data" method="post" onsubmit="return modifyAfficheFunction();">
			<div class="modal-header">
				<button type="button" class="close addusercancel" data-dismiss="modal">×</button>
				<h3>修改公告信息</h3>
...
<script type="text/javascript" src="/statics/localjs/gonggao.js"></script> 
