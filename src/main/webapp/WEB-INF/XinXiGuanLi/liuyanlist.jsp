...
<div>
	<ul class="breadcrumb">
		...
		<li><a href="/liuyan/loginLiuYanList">留言管理</a></li>
	</ul>
</div>
			...
			<div class="row-fluid sortable">		
				<div class="box span12">
					...
					<div id="tianye-zuo" style="float: left; padding-top: 8px; padding-left: 130px; font-size: 15px; color: blue;">共${page.zongShu }条留言 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  ${page.page }/${page.zongYeShu }页</div>
					  <ul>
					   <c:choose>
					  	...
					  	<c:otherwise>
					  	<li><a href="/liuyan/loginLiuYanList?currentpage=1" title="首页">首页</a></li>
					  	</c:otherwise>
					   </c:choose>
						<c:if test="${page.prevPages!=null}">
							<c:forEach items="${page.prevPages}" var="num">
								<li><a href="/liuyan/loginLiuYanList?currentpage=${num}"
						...
						<c:if test="${page.nextPages!=null}">
							<c:forEach items="${page.nextPages}" var="num">
								<li><a href="/liuyan/loginLiuYanList?currentpage=${num}" title="${num}">
								${num} </a></li>
							</c:forEach>
						</c:if>
						<c:if test="${page.zongYeShu !=null}">
							<c:choose>
						  	...
						  	<c:otherwise>
						  	<li><a href="/liuyan/loginLiuYanList?currentpage=${page.zongYeShu}" title="尾页">尾页</a></li>
						  	</c:otherwise>
						    </c:choose>
					    </c:if>
						<c:if test="${page.zongYeShu == null}">
					  ...
					  <div id="tianye-you" style="float: right; padding-top: 3px; margin-right: 250px; font-size: 15px; color: blue;">
					  	<form action="/liuyan/loginLiuYanList" method="post">
	 ...
	 <div class="modal hide fade" id="replyMessageDiv">
			...
			<ul id="replylist"></ul>
			<form action="/liuyan/huifu" enctype="multipart/form-data" method="post" onsubmit="return replyMessageFunction();">
			<div class="modal-body">
...
<script type="text/javascript" src="/statics/localjs/liuyanlist.js"></script> 
