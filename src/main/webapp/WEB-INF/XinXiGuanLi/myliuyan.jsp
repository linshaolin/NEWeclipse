...
<div>
	<ul class="breadcrumb">
		...
		<li><a href="/liuyan/myliuyan">我的留言</a></li>
	</ul>
</div>
		...
		<div class="pagination pagination-centered">
		  <ul>
		   <c:choose>
		  	...
		  	<c:otherwise>
		  	<li><a href="/liuyan/myliuyan?currentpage=1" title="首页">首页</a></li>
		  	</c:otherwise>
		   </c:choose>
			<c:if test="${page.prevPages!=null}">
				<c:forEach items="${page.prevPages}" var="num">
					<li><a href="/liuyan/myliuyan?currentpage=${num}"
			...
			<c:if test="${page.nextPages!=null}">
				<c:forEach items="${page.nextPages}" var="num">
					<li><a href="/liuyan/myliuyan?currentpage=${num}" title="${num}">
					${num} </a></li>
				</c:forEach>
			</c:if>
			<c:if test="${page.zongYeShu !=null}">
				<c:choose>
			  	...
			  	<c:otherwise>
			  	<li><a href="/liuyan/myliuyan?currentpage=${page.zongYeShu}" title="尾页">尾页</a></li>
			  	</c:otherwise>
			    </c:choose>
		    </c:if>
			<c:if test="${page.zongYeShu == null}">
	  ...
	  <div class="messsageform">
	  	<form action="/liuyan/addliuyan" method="post" onsubmit="return addMsg();">
...
<script type="text/javascript" src="/statics/localjs/myliuyan.js"></script> 
