...
$('.delgoodsinfo').click(function(e){
	...
	if(confirm("您确定要删除<"+d_goodsName+">这个商品吗？")){
		$.post("/goods/deletegoods",{'delId':d_id},function(result){
			if("success" == result){
				alert("删除成功！");
				window.location.href="/goods/logingoodspage";
			}else if("isstate" == result){
				alert("该商品处于上架状态，不能删除！");
...
$("#a_goodsSN").blur(function(){
	...
	if(agsn != ""){
		$.post("/goods/goosSNIsExit",{'goodsSN':agsn,'id':'-1'},function(result){
			if(result == "repeat"){
...
$("#m_goodsSN").blur(function(){
	...
	if(agsn != ""){
		$.post("/goods/goosSNIsExit",{'goodsSN':agsn,'id':$("#m_id").val()},function(result){
			if(result == "repeat"){
