...
$(".adduser").click(function(e){
	...
	$("#uniform-a_fileInputID span:first").html('请选择文件');
	$("#uniform-a_fileInputBank span:first").html('请选择文件');
});
...
$("#selectrole").change(function(){
	...
	if(sel_role == 2){
		$.post("/updateUser/getUserTyepe",{'s_role':sel_role},function(result){
			if(result != ""){
...
$("#m_logincode").blur(function(){
	...
	if(mlc != ""){
		$.post("/updateUser/logincodeisexit",{'loginCode':mlc,'id':$("#m_id").val()},function(result){
			if(result == "repeat"){
...
$("#a_uploadbtnID").click(function(){
					//0：标记是否为添加操作，	
					//a_fileInputID：上传的文件框
					//a_uploadbtnID：上传按钮
					//a_idPic：显示上传文件的div
					//a_fileInputIDPath：隐藏文本框，用于存放文件路径
...
function TajaxFileUpload(flag,t1,t2,t3,t4){   
	if($("#"+t1+"").val() == '' || $("#"+t1+"").val() == null){
		...
		$.ajaxFileUpload({ 
           url:'/updateUser/upload', 
           secureuri:false,
           fileElementId:t1,  //文件上传控件的id属性
           dataType: 'json',
           success: function(data) { 
        	   ...
        	   if(data == "1"){
        		   alert("上传图片大小不得超过50K！");
        		   //文件框显示回"请选择文件"
        		   $("#uniform-"+t1+" span:first").html('请选择文件');
        		 //标签选择器，文件按钮（浏览按钮）发生改变时
           ...
           error: function(date,XMLHttpRequest,textStatus,errorThrown,e) {  
        	  alert("上传失败！");
        	  alert("date="+date.responseText);
...
function delpic(id,closeSpan,uploadBtn,obj,picpath,picText,fileinputid){
	//传到后台处理
	$.post("/updateUser/delpic",{'id':id,'picpath':picpath},function(result){
		if("success" == result){
			alert("删除成功！");
			//隐藏文本框置空
			$('#'+picText).val('');
			//文件框显示回"请选择文件"
   			$("#uniform-"+fileinputid+" span:first").html('请选择文件');   
   			//显示上传文件的div置空
			...
			if("failed" == result){
				...
				if(roleId == '2'){
					...
					if(userType == null || userType == "")
						$("#m_selectusertype").append("<option value=\"\" selected=\"selected\">--请选择--</option>");
					$.post("/updateUser/getUserTyepe",{'s_role':roleId},function(result){
						if(result != ""){
							for(var i=0;i<result.length;i++){
...
$('.deluser').click(function(e){
	...
	if(confirm("您确定要删除【"+d_logincode+"】这个用户吗？")){
		//delete
		$.post("/updateUser/deluser",{'delId':d_id,'delIdCardPicPath':d_idcardpicpath,'delBankPicPath':d_bankpicpath,'delUserType':d_usertype},function(result){
			if("success" == result){
				alert("删除成功！");
				window.location.href="/updateUser/userlist";
