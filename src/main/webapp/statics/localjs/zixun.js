		...
		$.ajax({
			...
			success: function(result){
				if(result != "" && "success" == result){
					alert("删除成功！");
					window.location.href="/zixun/loginZiXunPage";
				}else if("failed" == result){
					alert("删除<"+title+">失败,请重试!");
				}else if("nodata" == result){
					alert("对不起，没有任何数据需要处理,请重试!");
...
$(".modifyInformationState").click(function(){
	...
	$.ajax({
		url: '/zixun/updateZiXunState',
		type: 'POST',
		data:{ziXunState:JSON.stringify(infoState)},
		dataType: 'html',
		timeout: 1000,
		error: function(){
